...
项目保留原来的 M0 平台基线。缩小后的 G0 只确认基础架构和三平台构建方向，
不再要求完成桌面封装。首个设计计算程序根据工作空间要求求解杆长，
再结合所得几何、负载以及最大速度和加速度，计算各关节的运动学与动力学需求。
这条计算链现已通过 `robot-engine-design` 直接可执行程序和 framed
`design.calculate` 请求开放，详见
[设计计算说明](docs/engineering/design-calculation.md)。
当前 sidecar 已实现有界 IPC framing、消息校验、结构化错误和
`engine.health` 请求。C++ 请求会话支持
并发任务、
...
CMake 3.22+ 和支持 C++20 的编译器。使用内置的 `dev` preset 配置和构建，再运行测试：
...
并校验已提交的协议 JSON。
脚本还会运行设计计算的可行/不可行算例、动力学解析参考、错误路径和可重复性测试。
可以直接查看结构化结果：
...
`kinematics.forward`。这里的参考模型是 M0 集成测试夹具，不是可直接制造的机器人设计。

M0 桌面 FK 骨架采用 Tauri 2 与 React，并将在 M5 最终封装时继续复用。它只暴露
最小命令面：WebView 可以查询引擎健康状态、请求正运动学或重启引擎，但没有直接
执行 shell 的权限。以下命令不会启动引擎，可用于安装依赖并验证前端：
...
图标为生成产物、不入库。首次构建前从 `1024x1024` 源 PNG 生成，否则 `generate_context!()`
会因缺 `icon.png` 失败：
