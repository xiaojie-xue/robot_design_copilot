...
int main() {
  ...
  while (true) {
    ...
    if (!frame.ok()) {
      std::cerr << "transport read error: " << frame_status_name(frame.status)
                << '\n';
      return 2;
    }

    const auto write_status = write_frame(std::cout, frame.payload);
    if (write_status != FrameStatus::ok) {
      std::cerr << "transport write error: " << frame_status_name(write_status)
                << '\n';
      return 3;
    }
  }
